...
- **Health checks**: Mandatory on every service (required for `--wait` deploys). Probe URLs must use `127.0.0.1`, never `localhost` (Alpine resolves `localhost` to `::1` first; IPv6 bridge bind fails on TrueNAS).
