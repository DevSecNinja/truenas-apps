...
| Compose validation | `docker compose -f src/<app>/compose.yaml config --quiet`                                     | each compose file         |
...
- **Networks**: Each app gets its own `<app>-frontend` network. Must be added to `src/traefik/compose.yaml`.
...
1. Create `src/<app>/compose.yaml` following ARCHITECTURE.md patterns
2. Create `src/<app>/secret.sops.env` listing required secret variables
3. Add the app's frontend network to `src/traefik/compose.yaml`
4. Add DNS records to `src/adguard/config/unbound/a-records.conf`
5. Update `README.md` (apps table + dataset list)
6. Update `docs/ARCHITECTURE.md` (UID/GID table, init container table)
7. Validate: `docker compose -f src/<app>/compose.yaml config --quiet`
...
Follow the conventions above. Only search the codebase for additional context if these instructions are incomplete or produce errors. When in doubt, model new files after the closest existing app in `src/`.
