...
| Local target                           | Network                | Routed by                            |
| -------------------------------------- | ---------------------- | ------------------------------------ |
| `https://traefik` (with `noTLSVerify`) | `cloudflared-frontend` | Traefik labels and middleware chains |

## Architecture

- **Image**: [cloudflare/cloudflared](https://github.com/cloudflare/cloudflared) (official)
- **Networks**: `cloudflared-frontend` (shared with Traefik on `svlazext`). Traffic is routed through Traefik, which applies its middleware chains before forwarding to any exposed backend.
...
1. Create a Cloudflare Tunnel in the [Zero Trust dashboard](https://dash.cloudflare.com/one/) under Networks -> Connectors
   ...
   4. You will now get to the 'Route tunnel' step. Configure public hostname rules to target `https://traefik` with `noTLSVerify` enabled and hit the Setup button.
