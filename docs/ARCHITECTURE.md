...
Alloy also scrapes **Home Assistant**'s built-in Prometheus integration at `home-assistant:8123/api/prometheus` (60s interval, bearer-token auth via `HOME_ASSISTANT_PROM_TOKEN` in `services/alloy/secret.sops.env`). Reachability is provided by joining Alloy to the `home-assistant-frontend` Docker network — declared `external: true` in `services/alloy/compose.yaml` and dropped on svlazext via `compose.svlazext.yaml` (HA is not deployed on svlazext).
