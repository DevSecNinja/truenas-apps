...
- **Prefer the `docker.io` (Docker Hub) copy when the same image is published on multiple registries.** Docker Hub exposes a per-tag push timestamp (`tag_last_pushed`) through its registry API, which Renovate reads to enforce the `minimumReleaseAge` gate (see [Contributing](CONTRIBUTING.md#update-timing-policy)). Registries like `ghcr.io`, LinuxServer's registry, and most OCI registries do not expose a push timestamp through the Docker Registry V2 API, and Renovate will not trust the maintainer-supplied `org.opencontainers.image.created` label because it can be spoofed. For images hosted only on `ghcr.io` (or similar), Renovate cannot enforce the age gate and opens update PRs immediately. This is a **tie-breaker among functionally equivalent copies**, not an absolute rule: a hardened `dhi.io` image, a single-registry image, or a hardened or smaller variant that only exists on `ghcr.io` still takes priority per the rules above
