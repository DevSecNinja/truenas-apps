...
Browser execution is currently disabled. Preserve historical screenshots and
watch settings; a restore must not re-enable an old vulnerable browser.
Explicitly stop any old browser container rather than relying on the inactive
profile. See [current HTTP-only operation](services/changedetection.md#browser-and-networks).
...
1. Suspend scheduled dccd deployment and any other automation that could
   ...
   an upgrade, while keeping browser execution disabled.
...
8. Sign in through SSO and verify settings, watch/tag counts, representative
   ...
   files or parse errors, then test a Basic HTTP watch and a notification.
   Manually select Basic HTTP for any restored browser-backed watch; do not
   silently migrate stored watch settings or enable Chrome to test the restore.
...
that the staged DHI browser works or is patched; current operation is
HTTP-only, with browser execution disabled.
...
- **changedetection browser services** (`changedetection-chrome`, `changedetection-browser-proxy`) — staged and disabled by default; read-only shared launcher/policy under `services/shared/config/browser/`, tmpfs scratch only, and no persistent browser/proxy state or database backup requirement. The app's `./data` file state remains covered separately.
...
- **Karakeep browser services** (`karakeep-chrome`, `karakeep-browser-proxy`) — staged and disabled by default; the same read-only shared launcher/policy and tmpfs-only scratch, with no persistent browser/proxy state. Karakeep's existing SQLite backup and asset coverage are unchanged.
